        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           global_step=rmidp.global_step, decay_steps=FLAGS.decay_steps,
                                                           decay_rate=FLAGS.decay_rate, staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                grads, vars = zip(*optimizer.compute_gradients(rmidp.loss))
                grads, _ = tf.clip_by_global_norm(grads, clip_norm=FLAGS.norm_ratio)
                train_op = optimizer.apply_gradients(zip(grads, vars), global_step=rmidp.global_step, name="train_op")
            ...
            # Summaries for loss
            loss_summary = tf.summary.scalar("loss", rmidp.loss)
            ...
            current_step = sess.run(rmidp.global_step)
            ...
            def train_step(x_batch_content, x_batch_question, x_batch_option, y_batch):
                ...
                feed_dict = {
                    ...
                    rmidp.dropout_keep_prob: FLAGS.dropout_keep_prob,
                    rmidp.is_training: True
                }
                _, step, summaries, loss = sess.run(
                    [train_op, rmidp.global_step, train_summary_op, rmidp.loss], feed_dict)
            ...
            def validation_step(x_val_content, x_val_question, x_val_option, y_val, writer=None):
                ...
                for batch_validation in batches_validation:
                    ...
                    step, summaries, scores, cur_loss = sess.run(
                        [rmidp.global_step, validation_summary_op, rmidp.scores, rmidp.loss], feed_dict)
            ...
            for batch_train in batches_train:
                ...
                current_step = tf.train.global_step(sess, rmidp.global_step)
