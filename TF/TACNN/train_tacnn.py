...
from text_tacnn import TextTACNN
...
def train_tacnn():
    ...
    # Build a graph and tacnn object
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=FLAGS.learning_rate,
                                                           global_step=tacnn.global_step, decay_steps=FLAGS.decay_steps,
                                                           decay_rate=FLAGS.decay_rate, staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                grads, vars = zip(*optimizer.compute_gradients(tacnn.loss))
                grads, _ = tf.clip_by_global_norm(grads, clip_norm=FLAGS.norm_ratio)
                train_op = optimizer.apply_gradients(zip(grads, vars), global_step=tacnn.global_step, name="train_op")
            ...
            # Summaries for loss
            loss_summary = tf.summary.scalar("loss", tacnn.loss)
            ...
            current_step = sess.run(tacnn.global_step)

            def train_step(x_batch_content, x_batch_question, x_batch_option, y_batch):
                """A single training step"""
                feed_dict = {
                    tacnn.input_x_content: x_batch_content,
                    tacnn.input_x_question: x_batch_question,
                    tacnn.input_x_option: x_batch_option,
                    tacnn.input_y: y_batch,
                    tacnn.dropout_keep_prob: FLAGS.dropout_keep_prob,
                    tacnn.is_training: True
                }
                _, step, summaries, loss = sess.run(
                    [train_op, tacnn.global_step, train_summary_op, tacnn.loss], feed_dict)
            ...
            def validation_step(x_val_content, x_val_question, x_val_option, y_val, writer=None):
                ...
                for batch_validation in batches_validation:
                    ...
                    feed_dict = {
                        ...
                        tacnn.input_x_option: x_batch_option,
                        tacnn.input_y: y_batch,
                        tacnn.dropout_keep_prob: 1.0,
                        tacnn.is_training: False
                    }
                    step, summaries, scores, cur_loss = sess.run(
                        [tacnn.global_step, validation_summary_op, tacnn.scores, tacnn.loss], feed_dict)
            ...
            for batch_train in batches_train:
                ...
                current_step = tf.train.global_step(sess, tacnn.global_step)
